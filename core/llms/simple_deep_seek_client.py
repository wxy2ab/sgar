...
def _default_context_window(model: str) -> int:
    """Best-effort total context window (tokens) for a DeepSeek model name.

    DeepSeek-V4 (``deepseek-v4-pro`` / ``deepseek-v4-flash``) ships a
    1,048,576-token (1M) window per DeepSeek's official spec; the legacy
    ``deepseek-chat`` / ``deepseek-reasoner`` were 64K. This is the default
    budget for token-budget-aware compaction — override via
    ``context_window_tokens`` if a deployment differs.
    """
    m = (model or "").lower()
    if "v4" in m:
        return 1_048_576
...
class SimpleDeepSeekClient(LLMApiClient):
    ...
    def __init__(self, api_key: str = "", base_url: str = "https://api.deepseek.com/",model:str = "deepseek-v4-flash",
        ...
        # DeepSeek prompt-cache + reasoning accounting. Cache-hit input tokens
        # bill at ~1/10th of a miss, so hit-rate is the single biggest cost
        # lever for long tool loops; reasoning tokens are billed as output.
        # Surfaced via get_stats() so callers can see whether caching works.
        self.cache_hit_token_count = 0
        self.cache_miss_token_count = 0
        self.reasoning_token_count = 0
        # Output-truncation observability: finish_reason == "length" means the
        # model hit max_tokens (for the reasoning client, often after the think
    ...
    def _build_request_kwargs(
        ...
        # Normalize SDK-object tool_calls (as ``_process_tool_response`` stores
        # them straight from a completion's ``message.tool_calls``) into plain
        # dicts. ``_sanitize_messages`` keys its orphan check on
        # ``isinstance(t, dict)``, so an object form slips past it and its
        # paired ``role=tool`` result gets dropped — the second tool-round then
        # 400s on DeepSeek. Dict form is the canonical wire shape both the SDK
        # serializer and the orphan check accept; already-dict entries are
        # untouched.
        for msg in messages:
            ...
            if tcs:
                msg["tool_calls"] = [
                    ...
                    else (tc.model_dump() if hasattr(tc, "model_dump") else tc)
                    for tc in tcs
                ]
    ...
    def _process_tool_response(self, response, tools: List[Dict[str, Any]], function_module: Any) -> str:
        ...
        try:
            ...
            if hasattr(response, 'choices') and response.choices:
                ...
                self._note_finish_reason(response)
                ...
                if hasattr(assistant_output, 'tool_calls') and assistant_output.tool_calls:
                    ...
                    try:
                        ...
                        self._note_finish_reason(second_completion)
    ...
    def _extract_usage_counts(self, usage: Any) -> Dict[str, int]:
        ...
        # Fold the two prompt-cache shapes the DeepSeek/OpenAI ecosystem uses
        # into one pair (ported from Reasonix normaliseUsage): DeepSeek puts
        # prompt_cache_{hit,miss}_tokens at the top of usage; OpenAI/MiMo nest
        # the hit under prompt_tokens_details.cached_tokens. Whichever side
        # reports non-zero wins; miss is derived when only hit is given.
        ...
        hit = _int(usage_dict.get("prompt_cache_hit_tokens"))
        miss = _int(usage_dict.get("prompt_cache_miss_tokens"))
        prompt_details = usage_dict.get("prompt_tokens_details") or {}
        if hit == 0 and isinstance(prompt_details, dict):
            hit = _int(prompt_details.get("cached_tokens"))
        if miss == 0 and hit > 0 and normalized_usage["prompt_tokens"] > hit:
            miss = normalized_usage["prompt_tokens"] - hit
        ...
        normalized_usage["cache_hit_tokens"] = hit
        normalized_usage["cache_miss_tokens"] = miss
    ...
    def _update_stats(self, usage: Any):
        ...
        self.cache_hit_token_count += usage_counts["cache_hit_tokens"]
        self.cache_miss_token_count += usage_counts["cache_miss_tokens"]
        self.reasoning_token_count += usage_counts["reasoning_tokens"]

    def _note_finish_reason(self, completion: Any) -> None:
        """Record the latest finish_reason and flag truncated output.

        ``length`` means the model hit ``max_tokens`` — the answer (or its
        JSON tail) is cut off. Surfacing it turns a silent truncated turn into
        an observable signal callers can act on (continue / compact / retry).
        """
        try:
        ...
        if finish_reason == "length":
            ...
            logger.warning(
                "⚠️ DeepSeek output truncated (finish_reason=length): hit "
                "max_tokens=%s; the response may be cut off mid-content.",
                self.max_tokens,
            )

    def get_stats(self) -> Dict[str, Any]:
        cached_input = self.cache_hit_token_count + self.cache_miss_token_count
        cache_hit_rate = (
            self.cache_hit_token_count / cached_input if cached_input else 0.0
        )
        return {
            ...
            "cache_hit_tokens": self.cache_hit_token_count,
            "cache_miss_tokens": self.cache_miss_token_count,
