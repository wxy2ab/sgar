...
更进一步说，`sgar` 想服务的不是“多一个 CLI”这件事，而是“让每个项目都拥有一个部署在业务环境里的内嵌 agent”这件事。这个 agent 要能够围绕真实业务数据持续做 `auto research`、`self-improving`、`self-operation`，而 `sgar` 提供的正是它所需的治理骨架与执行内核。
